...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-encode-directory", "directory"],
    ["x-encode-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-encode-directory")
  next.headers.delete("x-encode-workspace")
  return next
}

export const createOpencodeClient = createEncodeClient
...
export function createEncodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-encode-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-encode-workspace": config.experimental_workspaceID,
    }
  }
