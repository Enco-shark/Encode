 ...
 *
 * @throws if ENCODE_HOME is set but not an absolute path
 */
export function resolveENCODEHome(env: NodeJS.ProcessEnv = process.env): ResolvedPaths {
  const home = env.ENCODE_HOME
  if (home) {
    if (!path.isAbsolute(home)) {
      throw new Error(
        `ENCODE_HOME must be an absolute path, got: ${JSON.stringify(home)}`,
      )
    }
    return {
      mode: "ENCODE_home",
...
export namespace Global {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const { data, cache, config, state } = yield* Effect.sync(() => resolveENCODEHome())
