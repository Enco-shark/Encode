...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run encode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for encode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found encode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["Encode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting encode...")
        UI.println()

        const EncodeArgs = sessionId ? ["-s", sessionId] : []
        const EncodeProcess = Process.spawn(["Encode", ...EncodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await EncodeProcess.exited
        if (code !== 0) throw new Error(`Encode exited with code ${code}`)
      },
    })
  },
})
