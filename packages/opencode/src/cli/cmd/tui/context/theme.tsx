...
import encode from "./theme/encode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@encode-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light"; plain?: boolean }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = props.plain ? "system" : config.theme ?? kv.get("theme", "encode")
        draft.active = typeof active === "string" ? active : "encode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.encode, store.mode)
    })
...
function generateSystem(colors: TerminalColors, mode: "dark" | "light"): ThemeJson {
  ...
  const encodeBlue = RGBA.fromInts(0, 119, 182)

  return {
    theme: {
      // Primary colors
      primary: encodeBlue,
      secondary: ansiColors.blue,
