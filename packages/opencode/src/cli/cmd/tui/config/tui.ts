...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const projectFiles = Flag.ENCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ENCODE_TUI_CONFIG override, if set.
  if (Flag.ENCODE_TUI_CONFIG) {
    const configFile = Flag.ENCODE_TUI_CONFIG
