...
export const dict = {
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}encode.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/encode/tui.json{/highlight} を使います",
  ...
  "tui.tips.mcp_oauth": "Encode は認証が必要なリモート MCP サーバの OAuth を自動処理します",
  "tui.tips.custom_command":
    "{highlight}.encode/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.encode/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.formatter": "Encode は prettier、gofmt、ruff などでファイルを自動整形します",
  ...
  "tui.tips.lsp": "Encode はインテリジェントなコード解析に LSP サーバを使います",
  "tui.tips.custom_tool":
    "{highlight}.encode/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.encode/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  "tui.tips.plugin_notify": "プラグインでセッション完了時に OS 通知を送れます",
  "tui.tips.plugin_protect": "プラグインを書いて Encode が機密ファイルを読まないようにできます",
  "tui.tips.run": "{highlight}Encode run{/highlight} で非対話的なスクリプト実行ができます",
  "tui.tips.continue": "{highlight}Encode --continue{/highlight} で前回のセッションを再開します",
  "tui.tips.attach_cli": "{highlight}Encode run -f file.ts{/highlight} で CLI からファイルを添付できます",
  "tui.tips.format_json": "{highlight}--format json{/highlight} を使うとスクリプトで機械可読な出力を得られます",
  "tui.tips.serve": "{highlight}Encode serve{/highlight} を実行するとヘッドレスで Encode API にアクセスできます",
  "tui.tips.attach_server": "{highlight}Encode run --attach{/highlight} で起動中のサーバに接続します",
  "tui.tips.upgrade": "{highlight}Encode upgrade{/highlight} で最新バージョンに更新します",
  "tui.tips.auth_list": "{highlight}Encode auth list{/highlight} で設定済みプロバイダを確認します",
  "tui.tips.agent_create": "{highlight}Encode agent create{/highlight} でガイド付きエージェント作成を行います",
  "tui.tips.github_trigger": "GitHub の issue/PR で {highlight}/opencode{/highlight} を入力すると AI を起動します",
  "tui.tips.github_install": "{highlight}Encode github install{/highlight} で GitHub workflow を設定します",
  ...
  "tui.tips.theme_files": "{highlight}.encode/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.tips.debug_config": "{highlight}Encode debug config{/highlight} で設定の問題を調査します",
  ...
  "tui.dialog.login.Encode_free.success": "MiMo Auto (free) の準備完了 — デフォルトモデルを Encode/Encode-auto に設定しました",
  "tui.dialog.login.Encode_free.unavailable": "MiMo Auto (free) プロバイダーが読み込まれていません",
  ...
  "cli.providers.Encode_free.verifying": "MiMo Auto (free) チャネルを検証中...",
  "cli.providers.Encode_free.ready": "MiMo Auto (free) チャネル準備完了",
  "cli.providers.Encode_free.failed": "MiMo Auto (free) の自己診断に失敗",
  "cli.providers.Encode_free.default_set": "デフォルトモデルを Encode/Encode-auto に設定（1M コンテキスト、無料）",
  "cli.providers.Encode_free.usage_hint":
    "ログイン不要 — そのまま Encode を実行できます。有料/上位モデルを利用する場合は MiMo ブラウザログインを選択してください。",
