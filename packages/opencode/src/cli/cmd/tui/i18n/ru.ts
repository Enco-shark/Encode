...
export const dict = {
  ...
  "tui.tips.config_files":
    "Создайте {highlight}encode.json{/highlight} для серверных настроек и {highlight}tui.json{/highlight} для настроек TUI",
  "tui.tips.global_config":
    "Поместите настройки TUI в {highlight}~/.config/encode/tui.json{/highlight} как глобальные",
  "tui.tips.schema":
  ...
  "tui.tips.mcp_oauth":
    "Encode автоматически обрабатывает OAuth для удалённых MCP-серверов с авторизацией",
  "tui.tips.custom_command":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.encode/command/{/highlight} для повторно используемых пользовательских команд",
  ...
  "tui.tips.custom_agent":
    "Добавляйте файлы {highlight}.md{/highlight} в {highlight}.encode/agent/{/highlight} для специализированных AI-персон",
  ...
  "tui.tips.formatter":
    "Encode автоматически форматирует файлы с помощью prettier, gofmt, ruff и других инструментов",
  ...
  "tui.tips.lsp": "Encode использует LSP-серверы для интеллектуального анализа кода",
  "tui.tips.custom_tool":
    "Создавайте файлы {highlight}.ts{/highlight} в {highlight}.encode/tools/{/highlight}, чтобы определить новые LLM-инструменты",
  "tui.tips.tool_scripts":
    "Определения инструментов могут вызывать скрипты на Python, Go и других языках",
  "tui.tips.plugins":
    "Добавляйте файлы {highlight}.ts{/highlight} в {highlight}.encode/plugin/{/highlight} для подключения хуков событий",
  ...
  "tui.tips.plugin_protect":
    "Создайте плагин, который запретит Encode читать чувствительные файлы",
  "tui.tips.run":
    "Используйте {highlight}Encode run{/highlight} для неинтерактивных скриптов",
  "tui.tips.continue":
    "Используйте {highlight}Encode --continue{/highlight}, чтобы продолжить последнюю сессию",
  "tui.tips.attach_cli":
    "Используйте {highlight}Encode run -f file.ts{/highlight}, чтобы прикрепить файлы из командной строки",
  "tui.tips.format_json":
    "Используйте {highlight}--format json{/highlight} для машинно-читаемого вывода в скриптах",
  "tui.tips.serve":
    "Запустите {highlight}Encode serve{/highlight} для headless-доступа к API Encode",
  "tui.tips.attach_server":
    "Используйте {highlight}Encode run --attach{/highlight}, чтобы подключиться к запущенному серверу",
  "tui.tips.upgrade":
    "Выполните {highlight}Encode upgrade{/highlight}, чтобы обновиться до последней версии",
  "tui.tips.auth_list":
    "Выполните {highlight}Encode auth list{/highlight}, чтобы увидеть всех настроенных провайдеров",
  "tui.tips.agent_create":
    "Выполните {highlight}Encode agent create{/highlight}, чтобы создать агента в пошаговом режиме",
  ...
  "tui.tips.github_install":
    "Выполните {highlight}Encode github install{/highlight}, чтобы настроить GitHub-workflow",
  "tui.tips.github_oc":
  ...
  "tui.tips.theme_files":
    "Создавайте JSON-файлы тем в каталоге {highlight}.encode/themes/{/highlight}",
  ...
  "tui.tips.debug_config":
    "Выполните {highlight}Encode debug config{/highlight}, чтобы диагностировать проблемы конфигурации",
  "tui.tips.print_logs":
