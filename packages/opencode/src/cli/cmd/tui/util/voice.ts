...
export async function transcribeAudio(opts: {
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "api-key": opts.apiKey,
      "X-Mimo-Source": "encode-cli",
    },
    body: JSON.stringify({
      model: "Encode-v2.5-asr",
...
const VOICE_CONTROL_SYSTEM_PROMPT = `你是 Encode（AI 编程助手）的语音输入助手。用户通过语音向输入框口述消息，这些消息将发送给 Code Agent 执行编程任务。用户可能使用中文或英文。
