...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    ENCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    ENCODE_WORKSPACE_ID: config.id,
    ENCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.ENCODE_EXPERIMENTAL_WORKSPACES) return
