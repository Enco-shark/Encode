...
export async function indexFromDisk(
  ...
  // For CC files, derive type from frontmatter; Encode files keep loc.type from path.
  const finalType =
...
export async function reconcileMemory(
  ...
  let indexedCount = 0
  for (const p of EncodeFiles) {
    const loc = parsePath(p)
    if (!loc) {
      log.warn("path outside memory layout, skipping", { path: p })
      continue
    }
    const result = await indexFromDisk(p, loc, "Encode", indexed.get(p))
