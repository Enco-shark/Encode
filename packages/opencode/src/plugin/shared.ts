...
export async function checkPluginCompatibility(target: string, encodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(encodeVersion) || semver.major(encodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.encode
  if (typeof range !== "string") return
  if (!semver.satisfies(encodeVersion, range)) {
    throw new Error(`Plugin requires encode ${range} but running ${encodeVersion}`)
  }
}
