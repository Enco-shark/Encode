...
function url() {
  return Flag.ENCODE_MODELS_URL || "https://models.dev"
}
...
export const Data = lazy(async () => {
  const result = await Filesystem.readJson(Flag.ENCODE_MODELS_PATH ?? filepath).catch(() => {})
  ...
  if (Flag.ENCODE_DISABLE_MODELS_FETCH) return {}
...
if (!Flag.ENCODE_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
  void refresh()
  setInterval(
    async () => {
      await refresh()
