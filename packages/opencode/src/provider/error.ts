...
const MIMO_GATEWAY_PROVIDERS = new Set(["Encode"])

// Gateway error.code values worth relabeling: moderation (421) and
...
function message(providerID: ProviderID, e: APICallError) {
  return iife(() => {
    // Gateway: relabel known block codes and surface error.param (the real
    ...
    if (/^\s*<!doctype|^\s*<html/i.test(e.responseBody)) {
      if (e.statusCode === 401) {
        return "Unauthorized: request was blocked by a gateway or proxy. Your authentication token may be missing or expired — try running `encode auth login <your provider URL>` to re-authenticate."
      }
      if (e.statusCode === 403) {
