      ...
      push_caps: Schema.optional(
        Schema.Struct({
          ...
          design_decisions: Schema.optional(PositiveInt).annotate({
            description: "Token cap for Â§10 Design decisions section of checkpoint.md (writer-side budget validation). Default: 3000.",
          }),
          open_notes: Schema.optional(PositiveInt).annotate({
            description: "Token cap for Â§11 Open notes section of checkpoint.md (writer-side budget validation). Default: 800.",
          }),
        }),
      ).annotate({
        description:
  ...
  workflow: Schema.optional(
    Schema.Struct({
      ...
      maxLifecycleAgents: Schema.optional(Schema.Number).annotate({
        description:
          "Hard ceiling on total agents a single workflow run may spawn over its life. Default 1000. Over-cap agent() calls return null (graceful degradation). PER-RUN, not tree-wide: each child workflow has its own independent budget, so a deep nesting can spawn maxDepth Ã— this over the whole tree (concurrent in-flight is still bounded by maxConcurrentAgents).",
      }),
