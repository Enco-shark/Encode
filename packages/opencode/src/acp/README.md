...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for encode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal encode sessions
...
encode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 encode acp
```
...
- **Terminal Integration**: Full terminal support via encode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | encode acp
```
...
ACP sessions map cleanly to encode's internal session model:
