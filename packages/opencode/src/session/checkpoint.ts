...
// v4 â†?v5 spec rationale); maximum is a SOFT ceiling on backward
// expansion â€?i.e. when the natural tail is below the floor we expand
...
//
// 20K is the empirical sweet spot â€?observed compact output is ~20K,
...
function estimateMessageTokens(m: { parts: Array<{ type: string; [k: string]: unknown }> }): number {
  ...
  for (const p of m.parts) {
    ...
    // "free" â€?a 0 here would let the tail-boundary algorithm swallow the part
 ...
 *    return. Do NOT pull boundary forward â€?message-granularity truncation
 ...
 * thinking-block atomicity â€?this function does NOT need to.
 *
 * Edge cases:
...
export function computeBoundary(
  ...
  // boundary forward â€?see jsdoc rationale).
  if (tailSum >= TAIL_MAX_TOKENS) {
    return msgs[startIdx].info.id
  }

  // Tail too small â€?pull boundary earlier (include more history)
...
function renderSectionBudgets(budgets: Record<string, number>): string {
  ...
  if (entries.length === 0) {
    throw new Error("CHECKPOINT_SECTION_BUDGETS is empty â€?F43 substitution would produce an empty prompt block")
  }
  const cols = 3
 ...
 * dirs â€?without this, the model frequently invents legacy `/data/checkpoints/`
...
function composeWriterPrompt(input: {
  ...
  notesFile: string
  rangeDesc: string
  progressDiff: string  // Spec â‘?Chain 2: empty string when nothing to reconcile
}): string {
  return [
    "<system-reminder>",
    ...
    "",
    "========================================================================",
    "ABSOLUTE PATHS â€?USE THESE VERBATIM. NEVER COMPUTE, INFER, OR MODIFY.",
    "========================================================================",
    "",
    ...
    "If you find yourself typing '/data/checkpoints/' as a parent, STOP â€?that",
 ...
 *              settles. If a pending request already exists it's evicted â€?
 *              newest wins because its range is a strict superset of the
 *              older pending range, so the older one would just duplicate
 *              work. (F40)
 * - "skipped": the request was rejected outright â€?empty session, system-
...
export interface Interface {
   ...
   * recall reminder so it fires whenever there is anything to recall â€?
   ...
   * "(latest checkpoint has no Snapshot section)" placeholder â€?the full
...
interface WriterState {
  ...
  // pending values â€?the newest range is always a strict superset of the
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Plain Map in the layer closure â€?same approach as compaction.ts
    ...
    const tryStartCheckpointWriter: (
      ...
      // request â€?newest wins because its range is a strict superset of the
      ...
      // we project each ToolPart to both a tool_use and a tool_result block â€?
      ...
      // under <sid>/tasks/<id>/. Resolve projectID once HERE â€?Instance.current
      ...
      // Migrate legacy lowercase memory.md â†?MEMORY.md before templating/reading.
      ...
      // v6: spawn writer as subagent â€?shared sessionID, automatic
      ...
      // (Actor â†?SessionPrompt â†?SessionCheckpoint â†?Actor) layer cycle.
      const actor = spawnRef.current
      if (!actor) {
        log.warn("tryStartCheckpointWriter skipping â€?Actor service unavailable", { sessionID: input.sessionID })
        return "skipped" as const
      }

      // Axis B: branch forkContext shape on config.checkpoint.fork.
      // - true  â†?preserve existing prefix-cache parent-fork behavior
      //          (parent agent's system + tools, full slice up to watermark).
      // - false â†?cold-start: writer's own system + tools, delta slice since
      //          last_checkpoint_message_id (aligned past tool_use/tool_result).
      // See spec 2026-06-09-checkpoint-writer-child-session-and-no-fork-fallback-design.md Â§3.
      //
      ...
      // docs/superpowers/specs/2026-06-09-checkpoint-writer-child-session-and-no-fork-fallback-design.md Â§4.5.
      ...
      // empty-delta path fell through to spawn â†?runLoop's
      // `isForkAgent && !forkCtx â†?break` â†?settle watcher resolved success â†?
      ...
      if (!forkMode && delta.length === 0) {
        ...
        // silent no-op that would still advance the watermark on settle â€?
      ...
      // (ToolRegistry â†?SessionCheckpoint â†?ToolRegistry) layer cycle.
      ...
      const forkCtx: ForkContext | undefined = yield* (buildPrefix
        ? Effect.gen(function* () {
            ...
            // NOTE: parentAgentName guard is scoped to the forkMode:true branch only â€?
            ...
            if (forkMode) {
              if (!parentAgentName) {
                log.warn(
                  "tryStartCheckpointWriter: watermark has no agent, fork:true requires parent agent â€?falling back to no forkContext",
                  { sessionID: input.sessionID, endMessageID },
                )
                return undefined as ForkContext | undefined
              }
              // fork:true â€?preserve existing prefix-cache parent-fork behavior.
            ...
            // fork:false â€?cold-start: use the writer's own system + tools and
      ...
      // See spec 2026-06-09-checkpoint-writer-child-session-and-no-fork-fallback-design.md Â§2.
      ...
      const result = yield* actor.spawn({
        ...
        // and must see parentSessionID to re-derive the right paths â€?without
      ...
      // seconds â€?no race in practice. See spec Â§6.1.
      ...
      // to the layer's lifetime â€?no orphan fiber on shutdown.
      yield* Effect.gen(function* () {
        ...
        // for it. Errors are swallowed â€?the queued writer's failure should
    ...
    const waitForWriter = Effect.fn("SessionCheckpoint.waitForWriter")(function* (sessionID: SessionID) {
      ...
      // the prune retry watcher. AgentOutcome â†?WriterOutcome translation:
      // success â†?"success", failure / cancelled â†?"failure".
    ...
    const renderRebuildContext = Effect.fn("SessionCheckpoint.renderRebuildContext")(function* (
      ...
      // render â€?return empty so the rebuild path is a no-op for them.
      // Note: agentID === "main" must pass through. After F49+F50 the main
      // agent's lastUser.agentID is "main" (DB rowâ†’info reconstruction in
      ...
      // would skip rebuild â†?fall through to F39 compaction â†?context loss.
      ...
      if (inFlight) {
        ...
        yield* Effect.race(
          ...
          Effect.sleep("60 seconds").pipe(
            Effect.tap(() => Effect.sync(() => log.warn("writer wait timeout â€?using on-disk checkpoint", { sessionID }))),
      ...
      // read) and Section 8 (FTS scope filter). ALS-bound â€?must be resolved
      ...
      lines.push(
        "The following blocks are auto-loaded from your session memory. They are already in your context â€?do not Read them as whole files. Use Grep for specific facts instead.",
      )
      lines.push("")
      ...
      if (tasks.length === 0) {
        ...
        const statusIcon = (s: string) =>
          ({ open: "ðŸ”µ", in_progress: "ðŸ”„", blocked: "ðŸŸ¡", done: "âœ?, abandoned: "â? })[s] ?? s
        const ledgerLines: string[] = []
        for (const t of topLevel) {
          ledgerLines.push(`- ${t.id} ${t.status} â€?${t.summary}`)
      ...
      if (actors.length > 0) {
        ...
        for (const a of actors) {
          const line = `- ${a.actorID} â€?${a.status}, "${a.description}" (agent=${a.agent})`
      ...
      lines.push("")
      lines.push(
        "Recent messages are preserved verbatim below â€?the assistant turn (and any tool results) you'll see is real history, not pseudo-content. Continue your task by responding to the most recent state.",
      )
      lines.push("")
      lines.push(
      ...
      // based on how the preserved tail ends: tool-calls â†?continue loop,
      // stop â†?check task spec before stopping again, tool â†?process results,
      // user â†?no addendum needed.
    ...
    const insertRebuildBoundary = Effect.fn("SessionCheckpoint.insertRebuildBoundary")(function* (input: {
      ...
      // is preserved â€?LLM still sees what action was taken; result body
      // becomes "[Old tool result content cleared]" via the converter at
      // message-v2.ts (ToolStateCompleted â†?output).
      ...
      //    pre-marker history).
      // 3. Else SKIP â€?the previous fallback of 0 would clear EVERY
...
// layer cycle Actor â†?SessionPrompt â†?SessionCheckpoint â†?Actor. The AppLayer
