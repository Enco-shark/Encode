...
export function recallHintLines(toolCfg: ToolStyleConfig | undefined): string[] {
  ...
  // memory has no shell form (no shell.parse) â†?always JSON.
  return [`- memory({ operation: "search", query: "<keyword>" })`, taskHint, actorHint]
}

/**
 * Cap on goal-driven main-loop re-entries per turn â€?the safety valve against
 ...
 * a different order (e.g. {url,format} vs {format,url}) â€?without this the
...
/**
 * Stable signature for an assistant step's *action* â€?the tool calls it made
 ...
 * reasoning as plain text parts â€?counting either would mask the repeated
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // (ToolRegistry â†?SessionCheckpoint â†?ToolRegistry). See prefix-capture-ref.ts.
    ...
    const predict = Effect.fn("SessionPrompt.predict")(function* (input: { sessionID: SessionID }) {
      ...
      // prediction prompt â€?its default ("output ONLY a thread title") would
...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
            ...
            .ask(
              {
                ...
                // have no human to answer a permission prompt â€?fail clean, don't hang.
    ...
    const sweepOrphanAssistants = Effect.fn("SessionPrompt.sweepOrphanAssistants")(function* (sessionID: SessionID) {
      ...
      const now = Date.now()
      // 1 hour â€?must exceed Task 1's chunkMs (300s) plus Task 2's
      // PERSISTENT_RETRY worst-case backoff (10 attempts Ã— 5 min cap =
    ...
    const lastAssistant = Effect.fnUntraced(function* (sessionID: SessionID, agentID?: string) {
      if (agentID !== undefined) {
        ...
        // Critical for concurrent same-session subagents â€?a session-wide lookup
    ...
    const runLoop: (sessionID: SessionID, agentID?: string, task_id?: string) => Effect.Effect<MessageV2.WithParts> = Effect.fn(
      ...
      function* (sessionID: SessionID, agentID?: string, task_id?: string) {
        ...
        // counter â€?do not add a second one. Local to runLoop so a fresh user
        // turn resets it (no cross-message pollution), same as outputLengthContinuations.
        let invalidContinuations = 0
        // structured-output ä¸“ç”¨ retryï¼šä¸Šé™æ¥è‡?lastUser.format.retryCountï¼ˆé»˜è®?2ï¼‰ï¼Œ
        // ä¸?invalidContinuationsï¼ˆgeneric invalidï¼‰åˆ†ç¦»ï¼Œäº’ä¸æ±¡æŸ“ã€‚å±€éƒ¨äºŽ runLoopï¼?
        // æ–°ä¸€è½®ç”¨æˆ?turn è‡ªåŠ¨å½’é›¶ã€?
        ...
        // This guarantees "no output-length continuation when a tool is involved" â€?
        ...
        // the active goal is satisfied. Not satisfied â†?inject the judge's
        ...
        const goalGate = Effect.fn("SessionPrompt.goalGate")(function* (lastUser: MessageV2.User) {
          ...
          if (verdict.ok || verdict.impossible) {
            ...
            // âœ?âŠ?result line before the indicator disappears. goal.clear also
        ...
        while (true) {
          ...
          // Skip provider-executed tool parts â€?those were fully handled within the
          ...
          if (step === 1 && !session.parentID) {
            ...
            // (prompt â†?app-runtime â†?AppLayer â†?SessionPrompt). Only loaded when a
            ...
            if (repeating) {
              ...
              if (
                ...
                lastUserMsg.parts.push({
                  ...
                  text: [
                    "<system-reminder>",
                    `Your last ${REPEATED_STEP_THRESHOLD} steps have been identical â€?you appear to be`,
          ...
          // Bounded computation agents (native + hidden â€?currently title,
          ...
          if (
            ...
            // Subagent overflow â†?per-actor compaction (lossy LLM summarization
            // scoped to the actor's (sessionID, agent_id) slice). Subagents
            // don't have checkpoints, so checkpoint+discard does not apply.
            // Gate must exclude agentID="main" â€?F49+F50 made main carry
            ...
            if (lastUser.agentID && lastUser.agentID !== "main") {
              ...
              // slice begins at the boundary marker â€?context is freed for the
            ...
            // F39: no checkpoint â€?fall back to compaction (LLM-driven lossy summary).
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            // agent identity â€?which would diverge from the parent and break the
            // prefix cache.
            ...
            // spawnRef to break the Actor â†?SessionPrompt â†?Actor layer cycle).
            ...
            if (isForkAgent) {
              ...
              // Note: fork uses `tools` from resolveTools (not `forkCtx.tools`) â€?runtime
              ...
              const result = yield* handle.process({
                ...
                // visible tool set as the parent â†?prompt-cache parity on the inherited
                ...
                // while the fork's is merge(writer.permission, parentPermission) â€?so a
            ...
            // intentionally don't use it here â€?the `tools` variable from `resolveTools`
            ...
            // Main runLoop: no watermark â€?LLM must see the full msgs list,
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      // /goal â€?set or clear a session-level stop-condition goal. The condition
