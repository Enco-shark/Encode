...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under encode.", () => {
    expect(paramToAttributeKey("name")).toBe("encode.name")
    expect(paramToAttributeKey("slug")).toBe("encode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["encode.sessionID"]).toBeUndefined()
    expect(attrs["encode.messageID"]).toBeUndefined()
    expect(attrs["encode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under encode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["encode.name"]).toBe("exa")
