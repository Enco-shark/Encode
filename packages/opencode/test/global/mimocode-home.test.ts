...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ENCODE-home-test-"))
  return {
    path: dir,
...
describe("ENCODE_HOME end-to-end", () => {
  test("sets all four paths under the profile root", async () => {
    await using tmp = await tmpdir()
    const result = await runWorker({ ENCODE_HOME: tmp.path })
  ...
  test("auto-creates all four subdirs under a nonexistent root", async () => {
    ...
    const result = await runWorker({ ENCODE_HOME: nested })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test("two profiles have fully isolated data (write in A, not visible in B, A survives restart)", async () => {
    ...
    const resultA = await runWorker({ ENCODE_HOME: a.path })
    ...
    // Run with profile B â€?marker must not exist in B's data dir
    const resultB = await runWorker({ ENCODE_HOME: b.path })
    ...
    // Run again with profile A â€?marker must still be there
    const resultA2 = await runWorker({ ENCODE_HOME: a.path })
  ...
  test("relative path causes startup failure with clear error", async () => {
    const result = await runWorker({ ENCODE_HOME: "./relative-path" })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.stderr).toMatch(/ENCODE_HOME must be an absolute path/)
  })
  ...
  test("empty string falls through to XDG behavior", async () => {
    ...
    expect(result.ok).toBe(true)
    if (!result.ok) return
    // Paths should reflect XDG layout (ends with "/ENCODE"), not ENCODE_HOME layout
    expect(result.paths.config).toBe(path.join(tmp.path, "config", "ENCODE"))
    expect(result.paths.data).toBe(path.join(tmp.path, "data", "ENCODE"))
  })
})
