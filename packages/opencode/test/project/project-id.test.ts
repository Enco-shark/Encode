...
describe("resolveMainGitDir", () => {
  test("returns .git directory for main repo", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-"))
  ...
  test("returns null for non-git directory", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-nogit-"))
    expect(resolveMainGitDir(tmp)).toBeNull()
    await fs.rm(tmp, { recursive: true })
  })

  test("resolves worktree to main .git", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-wt-"))
...
describe("resolveProjectId", () => {
  test("generates new UUID for git project without cache", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-new-"))
    await fs.mkdir(path.join(tmp, ".git"))
    const id = resolveProjectId(tmp)
    expect(id).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)
    const cached = await fs.readFile(path.join(tmp, ".git", "encode-project-id"), "utf-8")
    expect(cached.trim()).toBe(id)
    await fs.rm(tmp, { recursive: true })
  })

  test("reuses cached UUID for git project", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-cache-"))
    await fs.mkdir(path.join(tmp, ".git"))
    await fs.writeFile(path.join(tmp, ".git", "encode-project-id"), "fixed-uuid-1234")
    expect(resolveProjectId(tmp)).toBe(ProjectID.make("fixed-uuid-1234"))
    await fs.rm(tmp, { recursive: true })
  })

  test("generates new UUID for non-git directory", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "Encode-pid-nogit-new-"))
    const id = resolveProjectId(tmp)
    expect(id).toMatch(/^[0-9a-f]{8}-/i)
    const cached = await fs.readFile(path.join(tmp, ".encode-project-id"), "utf-8")
    expect(cached.trim()).toBe(id)
    await fs.rm(tmp, { recursive: true })
  })
})
