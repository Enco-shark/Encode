...
describe("memory.search with cc scope", () => {
  it.live("ranks CC and Encode hits in one result list", () =>
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(EncodeRoot, "global", "m.md"), "distinctivetoken from Encode"),
          )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() => fs.rm(EncodeRoot, { recursive: true, force: true }))
          yield* Effect.promise(() => fs.mkdir(path.join(EncodeRoot, "global"), { recursive: true }))
          yield* Effect.promise(() => fs.writeFile(path.join(EncodeRoot, "global", "m.md"), "tokenz"))
