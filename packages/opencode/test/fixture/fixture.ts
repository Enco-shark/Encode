...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const dirpath = sanitizePath(
    path.join(process.env["ENCODE_TEST_TMPDIR_ROOT"] ?? os.tmpdir(), "ENCODE-test-" + Math.random().toString(36).slice(2)),
  )
  ...
  if (options?.git) {
    ...
    await $`git config user.email "test@ENCODE.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "encode.json"),
      JSON.stringify({
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info> }) {
  return Effect.gen(function* () {
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@ENCODE.test")
