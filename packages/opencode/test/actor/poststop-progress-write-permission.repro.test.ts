...
//
//   - `general` has permission defaults ("*":"allow") โ?write/edit survive
//     Permission.disabled โ?the postStop "use the Write tool" nudge is satisfiable
//     โ?progress.md lands.
//   - `explore` has "*":"deny" (read-only) โ?write/edit are stripped from its
//     LLM-visible tool set by Permission.disabled โ?the same nudge references a
//     tool it cannot call โ?no file unless it falls back to bash.
//
...
const FIVE_SECTION_BODY = [
  ...
  "",
  "## ยง3 Files and code sections",
  "- readme.txt: read it",
  "",
  "## ยง4 Verbatim commands",
  "```",
  "(none)",
  "```",
  "",
  "## ยง5 Outcome and discoveries",
...
describe("postStop progress.md is gated by the subagent's write permission", () => {
  ...
  // agents.get reads config, which needs an Instance context โ€?so run it inside
  ...
  it.live("explore disables write/edit; general keeps them", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* () {
        ...
        // explore ("*":deny) strips write+edit โ?postStop nudge is unsatisfiable.
        ...
        // general ("*":allow) keeps write+edit โ?nudge is satisfiable.
  ...
  // ruleset is what governs โ€?general's defaults ("*":allow) keep write enabled.
  it.live("general bound to task_id writes progress.md when postStop re-prompts", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        // Turn 1 (spawn): finish WITHOUT writing the journal โ?postStop sees the
        ...
        // Turn 2 (postStop re-entry): obey the nudge โ€?write the 5-section file.
  ...
  // progress checker SKIPS โ€?no nudge, no wasted re-entry turns โ€?and no progress.md lands.
  ...
  it.live("explore bound to task_id skips the progress check (no journal, clean finish)", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        ...
        // (canWrite=false โ?checker skips), so one terminal turn is all that's consumed.
