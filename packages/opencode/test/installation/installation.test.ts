...
describe("installation", () => {
  describe("method", () => {
    test("detects npm when @encode/cli-ai is in npm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "npm" && args.includes("-g")) return "@encode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects pnpm when @encode/cli-ai is in pnpm list output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "pnpm" && args.includes("-g")) return "@encode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("detects bun when @encode/cli-ai is in bun pm ls output", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        (cmd, args) => {
          if (cmd === "bun" && args.includes("-g")) return "@encode/cli-ai@1.0.0"
          return ""
        },
      )
    ...
    test("returns unknown when no package manager has @encode/cli-ai", async () => {
      const layer = testLayer(
        () => jsonResponse({}),
        () => "",
      )
    ...
    test("reads version from npm registry for npm method", async () => {
      const layer = testLayer(
        (req) => {
          ...
          expect(req.url).toContain(encodeURIComponent("@encode/cli-ai"))
  ...
  describe("upgrade", () => {
    test("runs npm install with correct package and registry", async () => {
      ...
      expect(capturedArgs).toContain("@encode/cli-ai@2.0.0")
      expect(capturedArgs.find((a) => a.startsWith("--registry="))).toContain("registry.npmjs.org")
    })
