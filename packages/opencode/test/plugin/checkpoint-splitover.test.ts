...
describe("CheckpointSplitoverPlugin", () => {
  ...
  test("extract-required â†?buildExtractionReflection in reason", async () => {
    ...
    const projectID = tmpProjectID()
    await setupSession(sessionID, projectID)
    // Â§1 Active intent budget is 500 tokens (~2000 chars). Blow past it.
    const oversized = "## Â§1 Active intent\n" + "x ".repeat(3000) + "\n"
  ...
  test("extract-required wins over error", async () => {
    ...
    const projectID = tmpProjectID()
    await setupSession(sessionID, projectID)
    // Â§1 Active intent over budget AND checkpoint.md content present but
