...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "encode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "encode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "encode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "encode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["encode.json", "encode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "encode.json"),
        path.join(tmp.path, "encode.jsonc"),
        path.join(project, "encode.json"),
        path.join(project, "encode.jsonc"),
      ])
    })
  })
