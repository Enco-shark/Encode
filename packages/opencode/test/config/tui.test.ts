...
test("migrates tui-specific keys from ENCODE.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "ENCODE.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when ENCODE.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "ENCODE.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple ENCODE.json levels", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
...
test("project config takes precedence over ENCODE_TUI_CONFIG (matches ENCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      process.env.ENCODE_TUI_CONFIG = custom
    },
  })
...
test("ENCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from ENCODE_CONFIG", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
