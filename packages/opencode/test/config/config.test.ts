...
const managedConfigDir = process.env.ENCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "ENCODE.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "ENCODE.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "ENCODE.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["ENCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ENCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["ENCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["ENCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "ENCODE.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .ENCODE directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ENCODE")
...
test("loads agents from .ENCODE/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ENCODE")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .ENCODE/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ENCODE")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .ENCODE/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".ENCODE")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("ENCODE command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".ENCODE", "command", "dup.md"),
        `---
description: ENCODE version
---
from ENCODE`,
      )
    },
...
test("does not try to install dependencies in read-only ENCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.ENCODE_CONFIG_DIR
  process.env.ENCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ENCODE_CONFIG_DIR
    else process.env.ENCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ENCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .ENCODE config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".ENCODE")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .ENCODE config with different plugins
      await Filesystem.write(
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ENCODE config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "ENCODE.json"),
        JSON.stringify({
          $schema: "https://encode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".ENCODE")
...
// Note: preload.ts sets ENCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .ENCODE config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .ENCODE directory config enables it
      const opencodeDir = path.join(dir, ".ENCODE")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "ENCODE.json"),
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "ENCODE.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.ENCODE/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.ENCODE/plugin/demo.ts", "file:///project/.ENCODE/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.ENCODE/plugin/demo.ts"])
  })
...
describe("ENCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ENCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .ENCODE/ directories when flag is set", async () => {
    const originalEnv = process.env["ENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .ENCODE directory with a command
          const opencodeDir = path.join(dir, ".ENCODE", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .ENCODE should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ENCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ENCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ENCODE_CONFIG_DIR"]
      process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ENCODE_CONFIG_DIR"]
      } else {
        process.env["ENCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ENCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ENCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ENCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ENCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ENCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from ENCODE_CONFIG_DIR, not project
...
describe("ENCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in ENCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ENCODE_CONFIG_CONTENT"]
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["ENCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["ENCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in ENCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["ENCODE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.ENCODE.managed.test",
          PayloadType: "ai.ENCODE.managed",
