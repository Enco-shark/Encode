...
test("resolveModelRef resolves literals and groups (provider-aware)", async () => {
      ...
      await Bun.write(
        ...
        JSON.stringify({
          ...
          model_groups: {
            ...
            // default-only group (no beta member) â€?context beta must fall back to default
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      // 2. Group with no context provider â†?group default.
      const def = await resolve("ultra")
      expect(String(def.providerID)).toBe("alpha")
      expect(String(def.id)).toBe("alpha-1")

      // 3. Provider-aware: context beta â†?the beta member, not the default.
      ...
      // 4. Context provider absent from members â†?group default.
      ...
      // 6. Unknown group name â†?ModelGroupNotFoundError.
      expect(resolve("nope")).rejects.toThrow(/ProviderModelGroupNotFoundError/)
    },
  })
})

// Member-skip needs beta actually disabled, which means beta must not load â€?so
    ...
    init: async (dir) => {
      await Bun.write(
        ...
        JSON.stringify({
          ...
          model_groups: {
            // beta listed first but disabled â†?provider-aware scan skips it â†?default alpha
...
test("getSmallModel resolves the lite group provider-aware", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // Context beta has a lite member â†?provider-aware pick (not the default, not the heuristic).
      const beta = await getSmall(ProviderID.make("beta"))
      expect(String(beta?.providerID)).toBe("beta")
      expect(String(beta?.id)).toBe("beta-1")

      // Context alpha â†?its own member, which is also the group default.
...
//   - no `/`  â†?tier/group name â†?Info.modelRef (resolved provider-aware later)
//   - has `/` â†?literal provider/model â†?Info.model (parsed eagerly)
...
test("per-call model accepts both group name and literal provider/model", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // input.model = "ultra" (group name) â†?provider-aware: context beta picks the beta member.
      const group = await resolve("ultra", ProviderID.make("beta"))
      expect(String(group.providerID)).toBe("beta")

      // input.model = "beta/beta-1" (literal) â†?exactly that model, IGNORING the alpha context provider.
      ...
      // input.model = "alpha/alpha-1" (literal) â†?alpha, no context provider.
...
// configured model becomes all three tiers â€?zero-config never errors.
...
test("configured built-in tier wins over fallback; unconfigured coexists", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // ultra is configured â†?the configured group, NOT the default alpha.
      const ultra = await resolve("ultra")
      expect(String(ultra.providerID)).toBe("beta")
      expect(String(ultra.id)).toBe("beta-1")

      // standard is an unconfigured built-in â†?default model.
...
// structured `model` field. Pure schema parse â€?no Instance/LLM needed.
