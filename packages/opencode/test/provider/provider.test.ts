...
// â€?a configured `lite` group (provider-aware), else the built-in tier fallback
    ...
    init: async (dir) => {
      await Bun.write(
        ...
        JSON.stringify({
          ...
          // A lite group exists too â€?small_model must still win (first branch).
          model_groups: {
            lite: "small/small-lite",
          },
        }),
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".encode", "plugin")
...
test("opencode and opencode-go providers are disabled by EncodeFreeAuthPlugin", async () => {
  ...
  // EncodeFreeAuthPlugin always pushes opencode/opencode-go into disabled_providers,
  ...
  expect(providers[ProviderID.make("Encode")]).toBeDefined()
  expect(providers[ProviderID.make("Encode")].models[ModelID.make("Encode-auto")]).toBeDefined()
  expect(providers[ProviderID.make("Encode")].models[ModelID.make("Encode-auto")].limit.context).toBe(1_000_000)
  expect(providers[ProviderID.make("Encode")].models[ModelID.make("Encode-auto")].limit.output).toBe(128_000)
})
