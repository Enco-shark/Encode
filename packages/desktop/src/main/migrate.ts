...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.encode.desktop.dev",
  beta: "ai.encode.desktop.beta",
  prod: "ai.encode.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.encode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `encode.settings.dat` is special: it maps to the `encode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"encode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // encode.settings.dat → the electron settings store ("encode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "encode.global.dat").
  const storeName = filename === "encode.settings.dat" ? "encode.settings" : filename
