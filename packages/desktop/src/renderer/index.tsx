...
import "./styles.css"
import { useTheme } from "@encode-ai/ui/theme"
...
const deepLinkEvent = "encode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__ENCODE__ ??= {}
  const pending = window.__ENCODE__.deepLinks ?? []
  window.__ENCODE__.deepLinks = [...pending, ...urls]
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://encode.ai/favicon-96x96-v3.png",
      })
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("encode.global.dat").getItem("language")
