...
import { and, Database, eq, isNull, lt, or, sql } from "@encode-ai/console-core/drizzle/index.js"
import { KeyTable } from "@encode-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@encode-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@encode-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@encode-ai/console-core/util/date.js"
import { Identifier } from "@encode-ai/console-core/identifier.js"
import { Billing } from "@encode-ai/console-core/billing.js"
import { Actor } from "@encode-ai/console-core/actor.js"
import { WorkspaceTable } from "@encode-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@encode-ai/console-core/model.js"
import { Subscription } from "@encode-ai/console-core/subscription.js"
import { BlackData } from "@encode-ai/console-core/black.js"
import { UserTable } from "@encode-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@encode-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@encode-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@encode-ai/console-core/lite.js"
import { Resource } from "@encode-ai/console-resource"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-encode-session") ?? ""
    const requestId = input.request.headers.get("x-encode-request") ?? ""
    const projectId = input.request.headers.get("x-encode-project") ?? ""
    const ocClient = input.request.headers.get("x-encode-client") ?? ""
    logger.metric({
      is_stream: isStream,
      session: sessionId,
      request: requestId,
      client: ocClient,
      ...(model === "encode-v2-pro-free" && JSON.stringify(body).length < 1000 ? { payload: JSON.stringify(body) } : {}),
    })
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-encode-request")
          headers.delete("x-encode-session")
          headers.delete("x-encode-project")
          headers.delete("x-encode-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    const billingUrl = `https://encode.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://encode.ai/workspace/${authInfo.workspaceID}/members`
