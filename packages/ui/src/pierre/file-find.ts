...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("encode-find")
  api.delete("encode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("encode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("encode-find", new Highlight(...rest))
    return true
  }
