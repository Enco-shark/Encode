...
import { createOpencode, type ToolPart } from "@encode-ai/sdk"
...
console.log("🚀 Starting encode server...")
const opencode = await createOpencode({
  port: 0,
})
console.log("✅ Encode server ready")
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new encode session...")
    ...
    console.log("✅ Created encode session:", createResult.data.id)
  ...
  console.log("📝 Sending to encode:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Encode response:", JSON.stringify(result, null, 2))
